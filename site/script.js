...
const translations = {
  en: {
    ...
    'feat.validate.desc': 'PascalCase + duplicates + empty SVG + mask detection. Errors in Chinese.',
    'col.output': 'OUTPUT',
    ...
    'step.1.desc': 'Create a page with "icon" in its name. Add top-level frames ending with <code>Outlined</code>, <code>Filled</code>, or <code>Colored</code>. Name components in PascalCase. Keep masks out — outline the strokes left on Union shapes and turn off Clip content.',
    ...
    'install.download.cta': 'Open Releases page →',
    'install.download.hint': 'Download <code>figma-icon-manager-vX.Y.Z.zip</code> and unzip it',
    'install.import': 'In Figma: Plugins → Development → Import plugin from manifest...',
    'install.import.hint': 'Pick the <code>manifest.json</code> inside the unzipped folder',
    'install.dev.tag': 'FOR DEVELOPERS',
    'install.dev.desc': 'Build from source',
  ...
  zh: {
    ...
    'feat.validate.desc': '大驼峰命名 + 重名 + 空 SVG + mask 蒙版检测，错误提示中文显示。',
    'col.output': '输出',
    ...
    'step.1.desc': '创建名称包含 "icon" 的页面。顶层 Frame 以 <code>Outlined</code>、<code>Filled</code> 或 <code>Colored</code> 结尾。组件使用大驼峰命名。不要带蒙版——把 Union 拼合形状上残留的描边轮廓化，并关闭「裁剪内容」。',
    ...
    'install.download': '下载最新 Release',
    'install.download.cta': '打开 Releases 页 →',
    'install.download.hint': '下载 <code>figma-icon-manager-vX.Y.Z.zip</code> 并解压',
    'install.import': '在 Figma 中：Plugins → Development → Import plugin from manifest...',
    'install.import.hint': '选择解压后文件夹里的 <code>manifest.json</code>',
    'install.dev.tag': '开发者流程',
    'install.dev.desc': '从源码构建',
